...
import { ArrowRight, CalendarClock, Compass, FileText, RefreshCw, Route, Sparkles, TrendingUp } from 'lucide-react';

const SERIF = '"Iowan Old Style", "Palatino Linotype", "Book Antiqua", Georgia, serif';
const NAV_ORANGE = '#FF5A1F';

const MAP_PATH = 'M 52 214 C 104 206, 122 150, 180 150 C 238 150, 256 92, 308 74';
const MAP_STOPS = [
  { x: 52, y: 214, color: '#0F6E56', label: 'Safety' },
  { x: 180, y: 150, color: '#9A6D00', label: 'Play' },
  { x: 308, y: 74, color: '#DC4C0C', label: 'Challenge' },
];
...
export function WhenReady() {
  return (
    <div className="space-y-9 pb-16">
      ...
      <section
        className="relative overflow-hidden rounded-[32px] border border-[#ECE4D6] p-8 shadow-[0_30px_80px_-64px_rgba(26,22,20,0.45)] md:p-12"
        style={{ background: 'linear-gradient(135deg, #FBF7F0 0%, #FFFFFF 52%, #FAF5EC 100%)' }}
      >
        <div className="pointer-events-none absolute -left-24 -top-28 h-72 w-72 rounded-full bg-[#42A68E]/10 blur-2xl" />
        <div className="pointer-events-none absolute -bottom-32 right-4 h-80 w-80 rounded-full bg-[#FFAB00]/10 blur-2xl" />
        <div className="relative grid gap-10 lg:grid-cols-[1.04fr_0.96fr] lg:items-center">
          <div>
            <span className="inline-flex items-center gap-2 rounded-full bg-white px-4 py-1.5 text-[11px] font-bold uppercase tracking-[0.16em] text-[#A06A1E] shadow-[0_12px_30px_-22px_rgba(26,22,20,0.6)]">
              <Compass className="h-3.5 w-3.5" /> This reading is a map
            </span>
            <h2 className="mt-5" style={{ fontFamily: SERIF, fontSize: 'clamp(2rem, 3.5vw, 3.25rem)', lineHeight: 1.02, letterSpacing: '-0.04em', color: '#15110F' }}>
              You do not need to solve the whole architecture at once.
            </h2>
            <div className="mt-6 space-y-4 text-[15.5px] leading-relaxed text-[#4D4945]" style={{ fontWeight: 300 }}>
              <p>
...
function MapMotif() {
  return (
    <div className="relative mx-auto w-full max-w-[420px]">
      <div className="relative overflow-hidden rounded-[26px] border border-[#EEE7DB] bg-white/70 p-5 backdrop-blur-sm shadow-[0_24px_60px_-52px_rgba(26,22,20,0.5)]">
        <svg viewBox="0 0 360 260" className="h-auto w-full overflow-visible" role="img" aria-label="Your foundations as waypoints on a path">
          {/* faint contour lines for a map feel */}
          {[0, 1, 2, 3].map((i) => (
            <path
              key={i}
              d={`M -10 ${70 + i * 46} C 90 ${40 + i * 46}, 240 ${100 + i * 46}, 380 ${50 + i * 46}`}
              fill="none"
              stroke="#EFEAE0"
              strokeWidth="1.5"
            />
          ...
          {/* the route */}
          <path d={MAP_PATH} fill="none" stroke="#E2DACB" strokeWidth="6" strokeLinecap="round" />
          <motion.path
            d={MAP_PATH}
            fill="none"
            stroke="#C9B79A"
          ...
          {/* travelling marker */}
          <motion.g
            style={{ offsetPath: `path("${MAP_PATH}")`, offsetDistance: '0%' } as any}
            animate={{ offsetDistance: ['0%', '100%'] }}
            transition={{ duration: 5.2, repeat: Infinity, ease: 'easeInOut', repeatType: 'reverse' }}
          >
            <circle r="9" fill="#1A1614" opacity="0.08" />
            <circle r="4.5" fill="#1A1614" />
          </motion.g>
          {/* waypoints */}
          {MAP_STOPS.map((stop, i) => (
            <g key={stop.label}>
              <motion.circle
                ...
                fill={stop.color}
                opacity="0.14"
                animate={{ scale: [1, 1.35, 1], opacity: [0.16, 0.05, 0.16] }}
                transition={{ duration: 3.4, repeat: Infinity, ease: 'easeInOut', delay: i * 0.5 }}
                style={{ transformOrigin: `${stop.x}px ${stop.y}px` }}
              />
              <circle cx={stop.x} cy={stop.y} r="9" fill="#FFFFFF" />
              <circle cx={stop.x} cy={stop.y} r="6" fill={stop.color} />
              <text
                x={stop.x}
                y={stop.y - 22}
                textAnchor="middle"
                style={{ fontSize: 11, fontWeight: 800, letterSpacing: '0.08em', fill: stop.color, textTransform: 'uppercase' }}
              >
                {stop.label}
              </text>
            </g>
        ...
        <p className="mt-1 text-center text-[11px] font-semibold uppercase tracking-[0.16em] text-[#9A948D]">
